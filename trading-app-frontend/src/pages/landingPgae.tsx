...
const useOnScreen = (options: UseOnScreenOptions): [React.RefObject<HTMLDivElement>, boolean] => {
            ...
            <div className="relative z-10 flex flex-col min-h-screen">
                ...
                <main className="flex-grow flex flex-col justify-center">
                    ...
                    <section className="text-center container mx-auto px-4 py-20">
                        ...
                        <AnimatedElement delay={200}>
                            <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-10">
                                Trade with precision on the NSE & BSE. Access real-time data and powerful tools built for the modern Indian investor.
                            </p>
                        </AnimatedElement>
                        <AnimatedElement className="flex flex-col sm:flex-row items-center justify-center gap-4" delay={400}>
                            <button className="bg-amber-400 text-black font-bold w-full sm:w-auto px-8 py-3 rounded-lg hover:bg-amber-300 transition-all duration-300 hover:scale-105 transform hover:shadow-lg hover:button-glow hover:cursor-pointer"
                    ...
                    <section className="container mx-auto px-4 mt-10 md:mt-16">
                         <AnimatedElement className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 max-w-4xl mx-auto" delay={600}>
                            {indices.map((index) => (
                                <div key={index.name} className="bg-black/20 backdrop-blur-md border border-white/10 p-4 rounded-lg text-center md:text-left transition-all duration-300 hover:border-white/20 hover:bg-black/30">
                                    <div className="flex flex-col md:flex-row justify-between items-center">
                                        ...
                                        <p className={`mt-2 md:mt-0 text-md font-semibold ${index.up ? 'text-green-500' : 'text-red-500'}`}>{index.change}</p>
                                    </div>
                                </div>
                            ))}
                        </AnimatedElement>
