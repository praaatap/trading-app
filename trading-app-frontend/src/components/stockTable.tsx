...
const StockRow: FC<StockRowProps> = React.memo(({ stock, index, inWatchlist, onToggleWatchlist, onStockSelect }) => {
    ...
    useEffect(() => {
        if (prevPrice !== undefined && prevPrice !== stock.ltp) {
            const newClass = stock.ltp > prevPrice ? 'animate-flash-green' : 'animate-flash-red';
    ...
    return (
        ...
        >
            ...
            <td className="px-2 sm:px-4 py-3 text-right w-24">
                <button onClick={(e) => e.stopPropagation()} className="bg-amber-400 text-black font-bold px-4 py-1.5 rounded-md text-xs opacity-0 group-hover:opacity-100 transition-all duration-150 hover:bg-amber-300">
                    Trade
                </button>
            </td>
        </tr>
    ...
    return (
        <div className="overflow-x-auto">
            <style>{`
                ...
                .animate-flash-red { animation: flash-red 1.5s ease-out; }
            `}</style>
            <table className="w-full text-sm text-left">
                <thead className="text-xs text-gray-400 uppercase">
                    <tr>
                        {tableHeaders.map((header) => (
                            <th key={header} className={`px-2 sm:px-4 py-4 font-medium 
